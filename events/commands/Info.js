...
const { guildId: devGuildId, ownerId } = require('../../setup/config.json');
        ...
        do(cmd) {
            ...
            if (!catName) {
                const embed = {
                    ...
                    color: 'RANDOM',
                    fields: [],
                    footer: { text: '貼心提醒：可使用「/commands <類別代號>」查詢各類別的指令' },
                };
                ...
                for (const file of cmdFiles) {
                    ...
                    const availableCmds = cmd.guildId === devGuildId ? cmds.filter((k) => !rawCat[k].off && (!rawCat[k].ownerOnly || cmd.user.id === ownerId)) : cmds.filter((k) => !rawCat[k].off && !rawCat[k].test && (!rawCat[k].ownerOnly || cmd.user.id === ownerId));
                    embed.fields.push({
